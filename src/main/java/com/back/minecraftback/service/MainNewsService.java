...
public class MainNewsService {
    ...
    public void saveAll(List<NewsDTO> saveNewsDTO) {
        mainNewsRepository.saveAll(saveNewsDTO
                .stream()
    ...
    private void handleNewEntity(NewsDTO dto, MainNewsEntity entity) {
        if (hasImage(dto)) {
    ...
    private void handleExistingEntity(NewsDTO dto, MainNewsEntity entity) {
        if (missingImage(dto)) {
